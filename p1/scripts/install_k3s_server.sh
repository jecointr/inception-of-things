...
# Install K3s server (controller node)
# Optional env vars:
#   K3S_TOKEN   -> cluster shared token
#   K3S_NODE_IP -> server node IP
...
# Install only what is needed, and only if missing
if ! command -v curl >/dev/null 2>&1; then
  apt-get update
  apt-get install -y --no-install-recommends curl ca-certificates
fi

INSTALL_K3S_EXEC="server --node-ip=${K3S_NODE_IP} --write-kubeconfig-mode=644 --token=${K3S_TOKEN}"
...
echo "[INFO] Waiting for Kubernetes API /ping endpoint..."
for i in $(seq 1 90); do
  if curl -kfsS https://127.0.0.1:6443/ping >/dev/null 2>&1; then
    echo "[INFO] Kubernetes API is responding"
    break
  fi
  sleep 2
  if [ "${i}" -eq 90 ]; then
